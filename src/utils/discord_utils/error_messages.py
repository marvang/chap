"""
Discord Integration - Error Messages
Handles error and alert notifications.
"""

import discord
from typing import Dict, Any
from .core import _safe_send, _create_embed


def send_error_alert_message(channel_id, error_type: str, error_msg: str, context: Dict[str, Any]) -> bool:
    """
    Send general error alert notification.

    Args:
        channel_id: Discord channel ID
        error_type: Type of error (e.g., "challenge_error", "container_error")
        error_msg: Error message
        context: Context dict with optional keys:
            - challenge: Challenge name
            - experiment_id: Experiment ID
            - iteration: Current iteration
            - Any other relevant context

    Returns:
        True if successful, False otherwise

    Example:
        >>> send_error_alert_message(
        ...     channel_id="123456789",
        ...     error_type="challenge_error",
        ...     error_msg="Docker container failed to start",
        ...     context={
        ...         "challenge": "vm0",
        ...         "experiment_id": "20250527_143022"
        ...     }
        ... )
    """
    if not channel_id:
        return False

    # Truncate error message if too long
    if len(error_msg) > 500:
        error_msg = error_msg[:497] + "..."

    # Build fields from context
    fields = [
        {"name": "Error Type", "value": error_type, "inline": True}
    ]

    if "challenge" in context:
        fields.append({"name": "Challenge", "value": context["challenge"], "inline": True})

    if "experiment_id" in context:
        fields.append({"name": "Experiment", "value": context["experiment_id"], "inline": True})

    if "iteration" in context:
        fields.append({"name": "Iteration", "value": str(context["iteration"]), "inline": True})

    # Add error message
    fields.append({"name": "Error", "value": f"```{error_msg}```", "inline": False})

    embed = _create_embed(
        title="❌ Error Alert",
        description="An error occurred during experiment execution",
        color=discord.Color.red(),
        fields=fields
    )

    return _safe_send(channel_id, embed=embed)


def send_llm_error_message(channel_id, error_msg: str, context: Dict[str, Any]) -> bool:
    """
    Send LLM API error alert notification.

    Args:
        channel_id: Discord channel ID
        error_msg: Error message from LLM API
        context: Context dict with optional keys:
            - challenge: Challenge name
            - iteration: Current iteration
            - model: Model name
            - experiment_id: Experiment ID

    Returns:
        True if successful, False otherwise

    Example:
        >>> send_llm_error_message(
        ...     channel_id="123456789",
        ...     error_msg="API rate limit exceeded",
        ...     context={
        ...         "challenge": "vm0",
        ...         "iteration": 15,
        ...         "model": "x-ai/grok-4.1-fast:free"
        ...     }
        ... )
    """
    if not channel_id:
        return False

    # Truncate error message if too long
    if len(error_msg) > 500:
        error_msg = error_msg[:497] + "..."

    # Build fields from context
    fields = []

    if "challenge" in context:
        fields.append({"name": "Challenge", "value": context["challenge"], "inline": True})

    if "model" in context:
        fields.append({"name": "Model", "value": context["model"], "inline": True})

    if "iteration" in context:
        fields.append({"name": "Iteration", "value": str(context["iteration"]), "inline": True})

    if "experiment_id" in context:
        fields.append({"name": "Experiment", "value": context["experiment_id"], "inline": True})

    # Add error message
    fields.append({"name": "Error", "value": f"```{error_msg}```", "inline": False})

    embed = _create_embed(
        title="🤖❌ LLM API Error",
        description="LLM API call failed - experiment may be stopping",
        color=discord.Color.red(),
        fields=fields
    )

    return _safe_send(channel_id, embed=embed)


def send_command_timeout_message(channel_id, command: str, timeout_seconds: int, context: Dict[str, Any]) -> bool:
    """
    Send command timeout warning notification.

    Args:
        channel_id: Discord channel ID
        command: Command that timed out (will be truncated if long)
        timeout_seconds: Timeout threshold in seconds
        context: Context dict with optional keys:
            - challenge: Challenge name
            - iteration: Current iteration
            - experiment_id: Experiment ID

    Returns:
        True if successful, False otherwise

    Example:
        >>> send_command_timeout_message(
        ...     channel_id="123456789",
        ...     command="nmap -sV -p- 192.168.5.0",
        ...     timeout_seconds=150,
        ...     context={
        ...         "challenge": "vm0",
        ...         "iteration": 15,
        ...         "experiment_id": "20250527_143022"
        ...     }
        ... )
    """
    if not channel_id:
        return False

    # Truncate command if too long
    if len(command) > 200:
        command = command[:197] + "..."

    # Build fields from context
    fields = [
        {"name": "Timeout", "value": f"{timeout_seconds}s", "inline": True}
    ]

    if "challenge" in context:
        fields.insert(0, {"name": "Challenge", "value": context["challenge"], "inline": True})

    if "iteration" in context:
        fields.append({"name": "Iteration", "value": str(context["iteration"]), "inline": True})

    if "experiment_id" in context:
        fields.append({"name": "Experiment", "value": context["experiment_id"], "inline": True})

    # Add command
    fields.append({"name": "Command", "value": f"```bash\n{command}\n```", "inline": False})

    embed = _create_embed(
        title="⏱️ Command Timeout",
        description="Command exceeded timeout threshold and was killed",
        color=discord.Color.orange(),
        fields=fields
    )

    return _safe_send(channel_id, embed=embed)


def send_empty_command_stop_message(channel_id, context: Dict[str, Any]) -> bool:
    """
    Send empty command stop notification.

    Args:
        channel_id: Discord channel ID
        context: Context dict with optional keys:
            - challenge: Challenge name
            - iteration: Current iteration
            - experiment_id: Experiment ID

    Returns:
        True if successful, False otherwise

    Example:
        >>> send_empty_command_stop_message(
        ...     channel_id="123456789",
        ...     context={
        ...         "challenge": "vm0",
        ...         "iteration": 8,
        ...         "experiment_id": "20250527_143022"
        ...     }
        ... )
    """
    if not channel_id:
        return False

    # Build fields from context
    fields = []

    if "challenge" in context:
        fields.append({"name": "Challenge", "value": context["challenge"], "inline": True})

    if "iteration" in context:
        fields.append({"name": "Iteration", "value": str(context["iteration"]), "inline": True})

    if "experiment_id" in context:
        fields.append({"name": "Experiment", "value": context["experiment_id"], "inline": True})

    embed = _create_embed(
        title="❌ Agent Stopped: Empty Commands",
        description="Agent provided empty command 3 times consecutively and was stopped",
        color=discord.Color.red(),
        fields=fields
    )

    return _safe_send(channel_id, embed=embed)


def send_docker_connection_error_message(channel_id, container_name: str, context: Dict[str, Any]) -> bool:
    """
    Send Docker connection error notification.

    Args:
        channel_id: Discord channel ID
        container_name: Name of container that failed to connect
        context: Context dict with optional keys:
            - challenge: Challenge name
            - experiment_id: Experiment ID

    Returns:
        True if successful, False otherwise

    Example:
        >>> send_docker_connection_error_message(
        ...     channel_id="123456789",
        ...     container_name="ctf-agent-pic-demo-kali",
        ...     context={
        ...         "challenge": "vm0",
        ...         "experiment_id": "20250527_143022"
        ...     }
        ... )
    """
    if not channel_id:
        return False

    # Build fields from context
    fields = [
        {"name": "Container", "value": container_name, "inline": True}
    ]

    if "challenge" in context:
        fields.insert(0, {"name": "Challenge", "value": context["challenge"], "inline": True})

    if "experiment_id" in context:
        fields.append({"name": "Experiment", "value": context["experiment_id"], "inline": True})

    embed = _create_embed(
        title="🐳❌ Docker Connection Failed",
        description="Failed to connect to Docker container - experiment cannot continue",
        color=discord.Color.red(),
        fields=fields
    )

    return _safe_send(channel_id, embed=embed)
