...
async def _async_create_category(experiment_id: str) -> Optional[str]:
    if not DISCORD_MAIN_BOT_TOKEN or not DISCORD_GUILD_ID:
        return None
    ...
    try:
        ...
        if DISCORD_PARENT_CATEGORY_ID:
            try:
                parent = await client.fetch_channel(int(DISCORD_PARENT_CATEGORY_ID))
            except:
                pass

        category_name = experiment_id
        category = await guild.create_category(
            name=category_name,
            position=parent.position + 1 if parent else None  # type: ignore[arg-type]
        )

        print(f"✅ Discord category created: {category.name} (ID: {category.id})")
        return str(category.id)

    except Exception as e:
        print(f"❌ Failed to create Discord category: {e}")
        return None
    finally:
        await client.close()
        await asyncio.sleep(0.25)
...
def create_experiment_category(experiment_id: str) -> Optional[str]:
    if not DISCORD_MAIN_BOT_TOKEN or not DISCORD_GUILD_ID:
        return None

    try:
        return _run_async(_async_create_category(experiment_id))
    except Exception as e:
        print(f"❌ Error creating Discord category: {e}")
        return None
